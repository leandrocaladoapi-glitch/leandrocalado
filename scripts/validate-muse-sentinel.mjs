...
const slug = 'meta-muse-sentinel-agent-security-harness';
...
for (const lang of langs) {
  ...
  assert(read(`${prefix(lang)}/ai-crime-files`).includes(`href="${route(lang)}"`), `${lang}: hub link`);
...
console.log(`Validated ${langs.length} Meta Muse Sentinel explainers, hubs, hreflang, sitemap, schemas and WebP (${dimensions.join('x')}, ${bytes.length} bytes).`);
